...
class DeepLabv3_plus(nn.Module):
    ...
    def forward(self, input):#input 1, 3, 512, 512
        ...
        x = nn.Upsample(size=(int(math.ceil(input.size()[-2]/4)), int(math.ceil(input.size()[-1]/4))), mode='bilinear', align_corners=True)(x)
        ...
        x = nn.Upsample(size=input.size()[2:], mode='bilinear', align_corners=True)(x)
